...
const consumer = await mkdtemp(join(tmpdir(), 'sqlite3-compat-consumer-'));
...
import sqlite3, {Database, verbose, OPEN_READONLY} from 'sqlite3-compat';
import {createRequire} from 'node:module';
import {runChecks} from './checks.mjs';
const cjs = createRequire(import.meta.url)('sqlite3-compat');
assert.equal(cjs.Database, Database);
assert.equal(cjs, sqlite3);
assert.equal(verbose, sqlite3.verbose);
assert.equal(OPEN_READONLY, 1);
console.log(JSON.stringify({esm: await runChecks(sqlite3), cjs: await runChecks(cjs), versions: process.versions}));\n`;
await writeFile(join(consumer, 'consumer.mjs'), script);
const checks = JSON.parse(run(process.execPath, ['consumer.mjs'], consumer));
for (const name of ['types.mts', 'types.cts']) {
  await cp(join(root, 'test', name), join(consumer, name));
}
const tsconfig = JSON.parse(await readFile(join(root, 'test/tsconfig.json'), 'utf8'));
